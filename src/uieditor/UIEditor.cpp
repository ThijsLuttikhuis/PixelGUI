...
UIEditor::UIEditor() {
    window = std::make_shared<Window>(1920, 1080, "PixelGUI Editor v1.0");
    window->initialize();
    ...
    name = "Scene1";
    pos = glm::vec2(150, 100);
    size = glm::vec2(100, 100);
    auto scene = std::make_shared<PG::Scene>(name, pos, size);
    ...
    sprite = std::make_unique<PG::Sprite>("arrow");
    key = GLFW_KEY_S;
    auto button2 = std::make_shared<PG::Button>(name, pos, size, std::move(sprite), key);
